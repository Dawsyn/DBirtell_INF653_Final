...
router.get('/states', async (req, res) => {
    ...
    const transformedStates = funfacts.map(state => ({
        ...
        funfacts: state.funfacts.size === 0 ? [] : Array.from(state.funfacts.values())
    }));


    fs.readFile(path.join(__dirname, '../model/statesData.json'), 'utf8', (err, data) => {
        if (err) {
        ...
        let states = JSON.parse(data);
            ...
            states.forEach(state => {
                let match = transformedStates.find(transformedState => transformedState.stateCode === state.code);
                if (match) {
        ...
        const nonContiguousStates = ["AK", "HI"];  
         ...
         if (isContig === 'false') {
            ...
            const resultStates = states.filter(state => nonContiguousStates.includes(state.code));
...
router.get('/states/:code', async (req, res) => {
    ...
    fs.readFile(path.join(__dirname, '../model/statesData.json'), 'utf8', (err, data) => {
        ...
        const state = states.find(state => state.code.toUpperCase() === stateCode);
...
router.patch('/states/:state/funfact', async (req, res) => {
    ...
    fs.readFile(path.join(__dirname, '../model/statesData.json'), 'utf8', async (err, data) => {
        if (err) {
            console.error("Error reading file:", err);
            return res.status(500).json({ 'message': 'Failed to load state data' });
        }
        ...
        const state = states.find(state => state.code.toUpperCase() === stateCode);
        if (!state) {
            return res.status(404).json({ 'message': 'State not found in JSON file' });
        }
        ...
        try {
            ...
            if(stateFromDB.funfacts.size === 0){
                return res.status(404).json({ 'message': `No Fun Facts found for ${state.state}` });
            }
            ...
            console.error(err);
            res.status(500).json({ 'error': 'Failed to update the funfact', 'details': err });
        }
    });
});
...
router.delete('/states/:state/funfact', async (req, res) => {
    ...
    if (index < 1) {
        return res.status(400).json({ 'message': 'Index must be a 1-based integer, not zero or negative.' });
    }
    ...
    try {
        ...
        if (!stateFromDB || !stateFromDB.funfacts || stateFromDB.funfacts.size === 0) {
            return res.status(404).json({ 'message': `No Fun Facts found for ${state.state}` });
        }
        ...
        const result = await State.findOneAndUpdate(
            { stateCode: stateCode }, 
            { $unset: { [`funfacts.${funfactIndex}`]: 1 } },
            { $pull: { funfacts: null } },
            { new: true } // Return the modified document
        );

        if (!result) {
            return res.status(404).json({ 'message': 'State not found or index out of range.' });
        }
        ...
        const funfact = Array.from(result.funfacts.values());
        ...
        console.error(err);
        res.status(500).json({ 'error': 'Failed to remove the funfact', 'details': err });
    }
});
